...
def prediction(Odometer, Year, Transmission, Fuel, Manufacturer, Drive, Car_type, Cylinders):

    num_attribs=['odometer', 'year']
    cat_attribs= ['cylinders', 'manufacturer', 'type', 'drive', 'transmission', 'fuel']

    #pre-process the input
    new_instance= pd.DataFrame({"odometer": Odometer, "year": Year, "cylinders": Cylinders, "manufacturer":Manufacturer, "type":Car_type, "drive":Drive, "transmission" : Transmission, "fuel" : Fuel}, index=[0])
    new_instance['cylinders'] = new_instance['cylinders'].apply(lambda x: x.split()[0])
...
if st.sidebar.checkbox("Predict Used Vehicle Price"):
    st.subheader("Calculate the expected price of your used vehicle or of what you're looking for:")
    st.markdown("The below prediction model uses decision tree regressor to calculated expected vehicle price.")
    # following lines create boxes in which user can enter data required to make prediction 
    left1, right1 = st.columns(2)
    Odometer = left1.number_input("Odometer", min_value = 0)
    Year= right1.number_input("Release Year", min_value= 2000, max_value=3000)

    left2, right2 = st.columns(2)
    Transmission= left2.selectbox("Transmission Type", sorted(df.transmission.unique()))
    Fuel= right2.selectbox("Fuel Type", sorted(df.fuel.unique()))
    
    left3, right3= st.columns(2)
    Manufacturer= left3.selectbox("Manufacturer", sorted(df.manufacturer.unique()))
    Drive= right3.selectbox("Drive Type", sorted(df.drive.unique()))
    
    left4, right4= st.columns(2)
    Car_type= left4.selectbox("Vehicle Type", sorted(df['type'].unique()))
    Cylinders= right4.selectbox("Number of Cylinders", sorted(df.cylinders.unique()))
    #result =""

    # # when 'Predict' is clicked, make the prediction and store it 
    if st.button("Predict"): 
        result = round(float(prediction(Odometer, Year, Transmission, Fuel, Manufacturer, Drive, Car_type, Cylinders))) 
        st.success('Your vehicle price is ${:,}.'.format(result))
